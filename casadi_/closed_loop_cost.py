...
def casadi_experiment(solver_type, init_state, goal_state, args):
    ...
    for i in range(num_steps):
        ...
        closed_loop_cost_value += dt * sol.value(opti.f)
...
#         state_logs, error_logs, closed_cost_values, goal_x, goal_theta, sim_iter = results[solver_type]
#
...
#
#         closed_loop_cost.append(closed_cost_values)
#         closed_loop_cost_msg = f'{solver_type} --- Mean: {np.mean(closed_cost_values):.2f} \n'
#         f.write(closed_loop_cost_msg)
#         print(closed_loop_cost_msg)
#
#     f.close()
#
#     y_values = np.arange(len(closed_loop_cost_values))
#     # Time
#     axs[3].bar(y_values, closed_loop_cost, labels=results.keys(), showfliers=False, whis=(0, 100))
...
#
#     init_theta = init_state['theta']
#     plot_results(results, f'Closed loop cost Simulation Results Angle {init_theta:.2f}')